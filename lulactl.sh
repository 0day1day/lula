...
l_start() {
	...
	pgrep -f 'bash.*lulad.sh' &> /dev/null

	# check for original lula instance
	kill -0 $pid 2> /dev/null
	if [ $? -eq 0 ]; then
		echo -e "$fail lula is already running."
		exit 1
	fi
}
...
l_stop() {
	if kill -0 $pid 2> /dev/null; then
		if kill -TERM $pid 2> /dev/null; then
			echo -e "$ok lula stopped"
		else
			echo -e "$fail cannot stop lula"
		fi
	else
		echo -e "$fail lula is not running"
	fi
}

l_status() {
	if kill -0 $pid 2>/dev/null; then
		echo -e "$ok lula is running"
	else
		echo -e "$fail lula is not running"
	fi
}
...
./lulad.sh &
sleep 1
pgrep -f 'bash.*lulad.sh' > "$pid_file"
sleep 2
update_pid
l_status
